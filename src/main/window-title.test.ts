...
describe('formatWindowTitle', () => {
  ...
  it('prepends a marker when provided', () => {
    expect(formatWindowTitle('MongoBuddy', '1.30.0', { marker: '🟦' })).toBe('🟦 MongoBuddy 1.30.0');
  });
  ...
  it('combines marker and location', () => {
    expect(formatWindowTitle('MongoBuddy', '1.30.0', { marker: '🟦', location: 'mydb.users' })).toBe(
      '🟦 mydb.users — MongoBuddy 1.30.0'
    );
  });
  ...
  it('omits the location when empty or undefined', () => {
    ...
    expect(formatWindowTitle('MongoBuddy', '1.30.0', { marker: '🟦', location: '' })).toBe('🟦 MongoBuddy 1.30.0');
  });
});
